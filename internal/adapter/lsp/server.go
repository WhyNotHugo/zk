...
func (s *Server) refreshDiagnosticsOfDocument(doc *document, notify glsp.NotifyFunc, delay bool) {
	...
	if diagConfig.WikiTitle == core.LSPDiagnosticNone && diagConfig.DeadLink == core.LSPDiagnosticNone && diagConfig.MissingBacklink == core.LSPDiagnosticNone {
...
func (s *Server) getMissingBacklinkDiagnostics(doc *document, notebook *core.Notebook, severity core.LSPDiagnosticSeverity) []protocol.Diagnostic {
	...
	for _, backlink := range missingBacklinks {
		diagSeverity := protocol.DiagnosticSeverity(severity)
